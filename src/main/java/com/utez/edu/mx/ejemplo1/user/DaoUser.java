...
public class DaoUser {
    ...
    public boolean createUser(BeanUser user) {
        ...
        try {
            ...
            pstm.setString(4, user.getPerson().getBirthdate());
    ...
    public List<BeanUser> getAll() {
        ...
        try {
            con = ConnectionMySQL.getConnection();
            pstm = con.prepareStatement("SELECT user.id, user.email, user.created_at, status.id AS status_id, status.name as status_name, person.id as person_id, person.name as person_name, person.surname as person_surname, person.lastname as person_lastname, person.birthdate as person_birthdate, person.phone as person_phone FROM user INNER JOIN status ON user.status_id = status.id INNER JOIN person ON user.person_id = status.id");
